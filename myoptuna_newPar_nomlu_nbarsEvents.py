...
def objective(trial):
    ...
    command = "root -l -b -q macro_newPar_nomlu_nbarsEvents.C+\(-1,"+ params +",10\) | grep double | awk '{print $2}' " 
...
def optuna_mc(n_trials=100, timeout=600): #quando fermare ottimizzazione
    ...
    print("\n")
    command = "root -l -b -q macro_newPar_nomlu_nbarsEvents.C+\(-1,"+ bestpar_str +",10\) | grep double | awk '{print $2}' " 
    print(command)
